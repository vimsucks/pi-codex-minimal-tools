...
} from "@vanillagreen/pi-tool-renderer/extensions/tool-renderer/generic.js";
...
export function createApplyPatchToolDefinition(options: { cwd?: string; allowAbsolutePaths?: boolean | ((cwd: string) => boolean); deferRendering?: boolean } = {}) {
	...
	if (!options.deferRendering) {
		definition.renderCall = renderApplyPatchCall;
		definition.renderResult = renderApplyPatchResult;
	}
	return definition;
}
