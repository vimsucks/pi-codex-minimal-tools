...
test("loadSettings merges user settings then project settings", () => {
	...
	mkdirSync(user, { recursive: true });
	mkdirSync(join(project, ".pi"), { recursive: true });
	writeFileSync(join(user, "settings.json"), JSON.stringify({ kendex: { extensionManager: { config: { "@vimsucks/pi-codex-minimal-tools": { autoEnable: false, imageOutputDir: "user-images", imageModel: "gpt-image-1" } } } } }));
	writeFileSync(join(project, ".pi", "settings.json"), JSON.stringify({ kendex: { extensionManager: { config: { "@vimsucks/pi-codex-minimal-tools": { autoEnable: true, imageOutputDir: "project-images", imageModel: "bad-model", directImageApiFallback: true } } } } }));
	const previous = process.env.PI_CODING_AGENT_DIR;
	process.env.PI_CODING_AGENT_DIR = user;
	try {
...
test("loadSettings skips project settings until project trust is recorded", () => {
	...
	mkdirSync(user, { recursive: true });
	mkdirSync(join(project, ".pi"), { recursive: true });
	writeFileSync(join(user, "settings.json"), JSON.stringify({ kendex: { extensionManager: { config: { "@vimsucks/pi-codex-minimal-tools": { autoEnable: false } } } } }));
	writeFileSync(join(project, ".pi", "settings.json"), JSON.stringify({ kendex: { extensionManager: { config: { "@vimsucks/pi-codex-minimal-tools": { autoEnable: true } } } } }));
	const previous = process.env.PI_CODING_AGENT_DIR;
	process.env.PI_CODING_AGENT_DIR = user;
	try {
